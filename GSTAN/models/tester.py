...
def multi_pred(sess, y_pred, seq, batch_size, n_his, n_pred, dynamic_batch=True):
    ...
    '''
    n = seq.shape[2]
    pred_list = np.zeros([1, n_pred, n, 1])
    ...
    # pred_list = np.reshape(pred_list, [-1, n_pred, n, 1])
    pred_list = pred_list[1:]
    print(pred_list.shape)
    return pred_list, pred_list.shape[0]


def model_test(inputs, batch_size, n_his, n_pred, inf_mode, load_path='./output/models_sw'):
    '''
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        evl = evaluation(x_test[0:len_test, 2*n_his:, :, :], y_test, x_stats)
        print('pre:',np.squeeze(x_test).shape)
        print('eval',evl.shape)
        evl = np.reshape(evl,[-1,3])       
        y_test1 = np.squeeze(y_test)
        x_test1 = np.squeeze(x_test[0:len_test, step_idx + n_his+n_his, :, :])
#        print('MAE:',np.mean(np.abs(x_test1[:,0,:]-y_test1[0,:,:])*146.48))
        for i in range(n_pred):
            np.savetxt(f'true/time_step{i+1}.csv', x_test1[:,i,:],delimiter=",")
            np.savetxt('pre/time_step1.csv', y_test1[0,:,:],delimiter=",")

        for ix in range(n_pred):
            te = evl[ix]
            print(f'Time Step {ix + 1}: MAPE {te[0]:7.3%}; MAE  {te[1]:4.3f}; RMSE {te[2]:6.3f}.')
        print(np.mean(evl, axis=0))
